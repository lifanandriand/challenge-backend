...
describe("Test login user", () => {
  ...
  it("should return a 400 error if password wrong", async () => {
    const res = await request(app)
      .post(loginUrl)
      .send({ email: "lifan@mail.com", password: "test12345" });
    expect(res.status).toBe(400);
...
// test user getUserBy
describe("Test getUserBy", () => {
  const getUserByUrl = "/api/v1/users/detail";

  it("should return a 401 if not login", async () => {
    const res = await request(app).get(getUserByUrl);
  ...
  it("should return a 200 if login", async () => {
    ...
    expect(res2.body.data).toHaveProperty("profile_picture");
  });
});